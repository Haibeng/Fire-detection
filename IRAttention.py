...
class CBAM(nn.Module):
    def __init__(self, channels, reduction_ratio=16, kernel_size=7):
        """
        CBAM模块：先进行通道注意力，再进行空间注意力。

        Args:
            channels (int): 输入特征图的通道数
            reduction_ratio (int): 通道注意力中MLP的缩减比例
            kernel_size (int): 空间注意力卷积的核大小
        """
    ...
    def forward(self, x):
        ...
        spatial_att = self.sigmoid_spatial(self.conv_spatial(concat))
...
class iRMB_CBAM(nn.Module):
    def __init__(self, dim_in, norm_in=True, has_skip=True, exp_ratio=1.0, norm_layer='bn_2d',
        ...
        self.attn_s = attn_s
        if self.attn_s:
            # 使用 CBAM 替代原 MultiHeadAtt 注意力模块
